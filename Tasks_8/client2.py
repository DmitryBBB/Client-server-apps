...
from common.veriables import ACTION, ACCOUNT_NAME, TIME, MESSAGE, SENDER, MESSAGE_TEXT, PRESENCE, USER, RESPONSE, ERROR, \
    DEFAULT_IP_ADDRESS, DEFAULT_PORT, DESTINATION, EXIT
...
def create_exit_message(account_name):
    # Функция создает словарь с сообщением о входе
    return {
        ACTION: EXIT,
        TIME: time.time(),
...
def print_help():
    # ФУнкция выводит справку по командам
...
def main():
    ...
    # Если имя пользователя не было заданоб необходимо запросить пользователя
    if not client_name:
        client_name = input('Введите имя ползователя: ')
    ...
    try:
        ...
        # Wathdog основной циклб если один из потоков завершен,
        # то значит или потеряно соединение или пользователь ввел exit.
